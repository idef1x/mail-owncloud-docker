if [ -z $SQLHOST ]
  then echo "using local SQL server..going to initialize"
    ...
    sleep 3
    # Create user and allow all access to SQL server
#    mysql -uroot -e "CREATE USER '${SQLUSR}'@'localhost' IDENTIFIED BY '$SQLPWD'"
    mysql -uroot -e "CREATE DATABASE owncloud"
    mysql -uroot -e "GRANT ALL PRIVILEGES ON *.* TO '${SQLUSR}'@'localhost' IDENTIFIED BY '$SQLPWD' WITH GRANT OPTION"
